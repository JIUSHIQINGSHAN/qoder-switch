// qoder-switch postinstall：从「平台包」复制本平台二进制。
//
// 平台分包（esbuild 模式）：二进制发布在独立 npm 包（qoder-switch-<platform>-<arch>），
// 主包声明为 optionalDependencies，安装时 npm 自动装好平台包，postinstall 只需复制——
// 不依赖 GitHub，国内镜像（npmmirror）也能稳定安装。
//
// 环境变量覆盖：
//   QODER_SWITCH_BINARY=<本地二进制路径>  本地开发/离线安装（直接复制，不联网）
//
// 说明：本脚本只做文件复制（fs），不执行任何子进程。
const fs = require("fs");
...
function fail(msg) {
  console.error(`qoder-switch install: ${msg}`);
  console.error(
    "安装失败。请确认安装了对应平台包（npm 会自动装），或设置 QODER_SWITCH_BINARY 指向本地二进制。",
  );
  process.exit(1);
}
...
function copyFrom(src) {
  ...
  console.log(
    `qoder-switch: 二进制就绪 → ${target} (${(size / 1048576).toFixed(1)}MB)`,
  );
}

async function main() {
  // 1) 本地二进制覆盖（开发/离线）
  if (process.env.QODER_SWITCH_BINARY) {
    const local = path.resolve(process.env.QODER_SWITCH_BINARY);
    if (fs.existsSync(local)) return copyFrom(local);
    return fail(`QODER_SWITCH_BINARY 指向的文件不存在: ${local}`);
  }

  // 2) 从平台包复制（node_modules/qoder-switch-<platform>-<arch>/bin/<file>）
  try {
    ...
    if (fs.existsSync(target)) {
      console.log("qoder-switch: 二进制已存在，跳过");
      return;
    }
    return fail(
