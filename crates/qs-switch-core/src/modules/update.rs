...
//!
//! 下载安装走 tauri-plugin-updater（前端 JS 插件直连，签名公钥在 tauri.conf.json）。
...
use crate::modules::config::{
    atomic_write_bytes, http_request_raw, http_request_with_proxy, now_secs, switch_root,
};

/// 应用当前版本（来自 Cargo.toml package.version）。
pub const APP_VERSION: &str = env!("CARGO_PKG_VERSION");
pub const GITHUB_OWNER: &str = "JIUSHIQINGSHAN";
pub const GITHUB_REPO: &str = "qoder-switch";
...
pub fn github_config_file() -> PathBuf {
    switch_root().join("github_config.json")
}
...
pub fn save_github_config(cfg: &Value) -> std::io::Result<()> {
    ...
    std::fs::create_dir_all(switch_root())?;
    atomic_write_bytes(
        &github_config_file(),
...
async fn fetch_manifest_version(
    ...
    headers.insert("User-Agent".to_string(), "qoder-switch".to_string());
...
/// `.../releases/tag/v0.1.4`）解析 tag。不跟随重定向，避免拉到 HTML 页面。
...
mod tests {
    ...
    #[test]
    fn updater_manifest_urls_cover_merged_then_platform() {
